...
pub const LlvmBackend = struct {
    ...
    pub fn linkWindows(self: *LlvmBackend, allocator: std.mem.Allocator, opts: link.WindowsLinkOptions) !void {
        _ = self;
        return link.windows(allocator, opts);
    }

    /// Dump the LLVM IR to stderr (debugging).
    pub fn dumpIr(self: *LlvmBackend) void { emit.dumpIr(&self.cg); }
